...
def analyze_false_negatives(detector, X_test, y_test, threshold=0.25):
    """
    分析漏报的攻击样本特征
    """
    _, test_errors, _ = detector.predict(X_test)
    
    # 找出漏报的攻击样本（真实攻击但未检测到）
    false_negatives = np.where((y_test == 1) & (test_errors <= threshold))[0]
    
    print("\n" + "="*60)
    print(f"漏报攻击样本分析 (阈值={threshold})")
    print("="*60)
    
    print(f"总攻击样本: {np.sum(y_test == 1)}")
    print(f"漏报数量: {len(false_negatives)}")
    print(f"漏报率: {len(false_negatives)/np.sum(y_test == 1)*100:.2f}%")
    ...
    if len(false_negatives) > 0:
        ...
        plt.axhline(y=threshold, color='green', linestyle='--', linewidth=2, label=f'阈值={threshold}')
        plt.ylabel('重构误差')
        plt.title('重构误差对比')
        plt.legend()
        plt.grid(True, alpha=0.3)
...
def run_dae_attack_detection():
    ...
    # new_threshold = 0.25
    # new_predictions = (test_errors > new_threshold).astype(int)
    
    # print(f"\n使用阈值 {new_threshold} 的结果:")
    # print(classification_report(y_test, new_predictions, target_names=['正常', '攻击']))
